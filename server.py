    ...
    try:
        ...
        if comanda == 'CONNECT':
            ...
            if adresa_client not in clienti_conectati:
                ...
                print(f"[SERVER] Mesaj nou publicat (ID={contor_id}) de {adresa_client}")
            ...
            if adresa_client not in clienti_conectati:
                ...
                if id_cautat not in mesaje:
                    raspuns = f"EROARE: Nu exista niciun mesaj cu ID={id_cautat}."
                elif mesaje[id_cautat]['autor'] != adresa_client:
                    raspuns = f"EROARE: Nu poti sterge mesajul cu ID={id_cautat}. Nu esti autorul."
                else:
                    del mesaje[id_cautat]
                    raspuns = f"OK: Mesajul cu ID={id_cautat} a fost sters."
                    print(f"[SERVER] Mesaj sters (ID={id_cautat}) de {adresa_client}")
        elif comanda == 'LIST':
            if adresa_client not in clienti_conectati:
                ...
                linii = [f"OK: Lista mesaje ({len(mesaje)} total):"]
                for mid, continut in mesaje.items():
                    linii.append(f"  [{mid}] {continut['text']}")
                raspuns = '\n'.join(linii)
