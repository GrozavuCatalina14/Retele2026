...
class FTPClient:
    ...
    def rename_file(self):
        ...
        # Show files on server first so user knows what's available
        self.list_files()
    ...
    def download(self):
        ...
        if response['status'] == 'error':
            ...
            # Save the received content to local_files/
            content = response.get('content', '')
            filepath = os.path.join(LOCAL_FILES_DIR, filename)
            try:
                with open(filepath, 'w') as f:
                    f.write(content)
                print(f"✓ File '{filename}' downloaded and saved to {LOCAL_FILES_DIR}/")
            except Exception as e:
    ...
    def edit_file(self):
        ...
        # Remove trailing empty line added by double-Enter
    ...
    def see_file_operation_history(self):
        ...
        choice = input("Enter file number or name (or press Enter for ALL history): ").strip()
        ...
        if response['status'] == 'error':
            ...
            if not history:
                label = f"'{filename}'" if filename else "any file"
                print(f"✗ No history found for {label}")
            else:
                label = f"'{filename}'" if filename else "all files"
                print(f"\n📋 Operation history for {label}:")
                print("-" * 40)
                for entry in history:
                    print(f"  [{entry['timestamp']}] {entry['operation'].upper()} — {entry['filename']}")
                print("-" * 40)
